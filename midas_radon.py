...
#----fejk clasa do testow WavePlusFake---------
class FakeWavePlus:
    def __init__(self):
        self.values = {
            "Humidity": 50.0,
    ...
    async def read_data(self):
        await asyncio.sleep(0.8 + random.uniform(0,0.6)) # symulacja opóźnienia odczytu danych
        # tutaj można dac losowe zmiany
        self.values["Humidity"] += 0.5
        self.values["Radon ST"] += 1
        self.values["Radon LT"] += 1
        self.values["Temperature"] += 0.5
        self.values["Pressure"] += 0.02
        self.values["CO2"] += 1
        self.values["VOC"]+=1
